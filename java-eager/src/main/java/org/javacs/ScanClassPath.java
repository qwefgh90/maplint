...
public class ScanClassPath {
    ...
    public static String[] JDK_MODULES = {
        "java.activation",
        "java.base",
        "java.compiler",
        "java.corba",
            ...
            try (var stream = Files.walk(moduleRoot)) {
                ...
                while (it.hasNext()) {
                    ...
                    if (relative.endsWith(".class") && !relative.contains("$")) {
                        ...
                        var qualifiedName = trim.replace('/', File.separatorChar).replace(File.separatorChar, '.');
    ...
    public static Set<String> classPathTopLevelClasses(Set<Path> classPath) {
        ...
        LOG.info(String.format("Found %d top classes in classpath", classes.size()));
