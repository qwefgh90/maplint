...
import org.javacs.lsp.custom.ExtendedSymbolInformation;
...
class JavaLanguageServer extends LanguageServer {
    ...
    private JavaCompilerService createCompiler() {
        ...
        // Otherwise, combine inference with user-specified external dependencies
            ...
            var infer = new SimpleEnvironmentDetectService(workspaceRoot, externalDependencies);

            javaReportProgress(new JavaReportProgressParams("Inferring class path"));
            classPath = infer.getClassPath();

            javaReportProgress(new JavaReportProgressParams("Inferring doc path"));
            var docPath = infer.getBuildDocPath();
...
//    @Override
//    public ClassType classInformation(Path path) {
//        var provider = new ExtendedSymbolProvider(compiler());
//        var classInformation = provider.getClassType(path);
//        provider.print(null, classInformation);
...
//    }
    ...
    public List<ExtendedSymbolInformation> documentTypeAndSymbol(DocumentSymbolParams params) {
        if (!FileStore.isJavaFile(params.textDocument.uri)) return List.of();
        var file = Paths.get(params.textDocument.uri);
        return null;//new ClassFinder(compiler()).documentSymbols(file);
    }
