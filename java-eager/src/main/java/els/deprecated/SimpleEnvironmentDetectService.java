...
public class SimpleEnvironmentDetectService {
    ...
    public SimpleEnvironmentDetectService(Path workspaceRoot, Collection<String> externalDependencies, Path mavenHome, Path gradleHome) {
    ...
    public SimpleEnvironmentDetectService(Path workspaceRoot, Collection<String> externalDependencies) {
        this(workspaceRoot, externalDependencies, defaultMavenHome(), defaultGradleHome());
    }

    public SimpleEnvironmentDetectService(Path workspaceRoot) {
    ...
    public Set<Path> resolveBuildDocs() {
        // externalDependencies
        if (!externalDependencies.isEmpty()) {
            var result = new HashSet<Path>();
            for (var id : externalDependencies) {
    ...
    private Path findGradleJar(Artifact artifact, boolean source) {
        ...
        var pattern =
                "glob:"
                        + String.join(
                                File.separator.equals("\\") ? File.separator+File.separator : File.separator,
